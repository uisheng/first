...
window.SpinningWheel = {
    ...
    create: function () {
        ...
        div.innerHTML = '<div id="sw-header"><div id="sw-cancel">取消</' + 'div><div id="sw-done">确认</' + 'div></' + 'div><div id="sw-slots-wrapper"><div id="sw-slots"></' + 'div></' + 'div><div id="sw-frame"></' + 'div><div id="sw-line"></div>';
    ...
    open: function () {
        ...
        var pageOffset = yoffset - 20;
        if(pageOffset < 0 ) {
            pageOffset = 0;
        }
        var actlen = pageOffset + 260;
        this.swWrapper.style.webkitTransitionTimingFunction = 'ease-out';
        this.swWrapper.style.webkitTransitionDuration = '400ms';
        this.swWrapper.style.webkitTransform = 'translate3d(0, -' + actlen + 'px, 0)';
    },
    ...
    addSlot: function (values, style, defaultValue,callback) {
        if (!style) {
            style = '';
        }
        ...
        if(callback && typeof callback == 'function'){
            this.slotCallback.push(callback);
        };
    },

    //编辑slot
    editSlot : function(slot,values){
        //this.scrollToValue(l, this.slotData[l].defaultValue); 

        var str=[];
        var $ul = this.swSlots.querySelectorAll('ul')[slot];
        if(!$ul){
            console.error('未能获取到slot');
            return false;
        }
        //重写数据
        this.slotData[slot].values = values;

        //重置HTML结构
        for(var i in values){
            str.push('<li>'+ values[i] +'</li>');
        }
    ...
    scrollEnd: function (e) {
        ...
        var callback = this.slotCallback[this.activeSlot],
            _this = this;
        if(callback){
            setTimeout(function(){
                callback.call(_this,_this.getSelectedValues());  
            },30);  
        };
